...
export function ChoiceStep({ question, subtext, options, value, detailValue, onSelect, onBlocked }: Props) {
  ...
  return (
    <div>
      ...
      <div className="mt-5 short:mt-4 grid gap-2.5">
        {options.map((opt) => {
          ...
          return (
            <button
              ...
              onClick={() => choose(opt)}
              className={clsx(
                "group flex items-center justify-between gap-4 rounded-xl border px-4 py-3 short:py-2.5 text-left transition-all",
                active
